...
class UE():
    def __init__(self, user_id, data_num):
        ...
        self.hits = 0  # number of the hits
        self.loc = np.zeros((1, 2))
        self.num_step = 0  # the number of step
        self.total_req = 0  # the number of request
        self.time_start = 0  # the time of the start
        self.delay_time = 0
        ...
        if self.req.state == 5:
            ...
            if self.req.state == 0:
                ...
                if self.req.e2u_size > 0:
                    ...
                    self.req.state = 4
                    if(self.req.hitstart == True):
                        self.req.hitend = True
                        self.req.hitstart = False
                        self.hits += 1
                        self.delay_time = time.time() - self.time_start    
...
class Env():
    ...
    def algorithm_step_forward(self, a, r_dim, b_dim):
    ...
    def text_render(self):
        ...
        for user in self.U:
            ...
            print("hit:", user.hits)
            print("total_req:", user.total_req)
            if(user.total_req > 0):
                print("hit_rate:", user.hits / user.total_req)
                self.hitlist_dict[user.user_id] = user.hits/user.total_req
            res = 0
            for val in self.hitlist_dict.values():
                res += val
            if(len(self.hitlist_dict) > 0):
                res = res / len(self.hitlist_dict)
                print("hit_rate_avg:", res)
    ...
    def delay_handler(self):
        for user in self.U:
            ...
            if (user.delay_time!= 0):
                self.delay_list[user.user_id] = user.delay_time

    def hitlist_clear(self):
        self.hitlist_dict.clear()
    def delay_list_clear(self):
        self.delay_list.clear()

    def get_hitlist_avg(self):
        res = 0
        for val in self.hitlist_dict.values():
            res += val
        if(len(self.hitlist_dict) > 0):
    ...
    def get_delay_avg(self):
        return sum(self.delay_list.values())/len(self.delay_list)
    ...
    def update_tree(self, episode, reward, hit_rate, avg_delay):
        self.canvas.update_tree(episode, reward, hit_rate, avg_delay)
